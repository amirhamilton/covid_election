...
To assist with understanding the national distribution of *R*, I have created a sample density plot for November 4 shown below. The median *R* is above zero.
...
When percentage of Trump votes per county (trump.percent) was predicted it was found that Rt_plot on election day (Nov 4) was a significant  predictor (t = 2.892, p = 0.00388). The overall model fit is R^2 = 0.0062. The difference in means is 0.023.
...
As for the other dates, some things I would like to highlight are that the 
...
When percentage of Biden votes per county (biden.percent) was predicted it was found that *R* (Rt_plot) on election day (Nov 4) was a significant negative/inverse predictor (t = -2.621, p = 0.00887). The overall model fit is R^2 = 0.005097, and the difference in means is -0.020582.

Because the p-value is smaller than the standard threshold of alpha = .05 we can reject the null hypothesis that the relationship of biden.percent to *R* on Election Day is zero. In comparison to the relationship between trump.percent and *R*, biden.percent is less strongly related to *R*.
