...
## Boxplot of Rt_plot on Election Day
```{r}
ggplot(data = cc_nov_4, aes(x = Rt_plot)) +
  geom_density()
```

## Correlation Between Rt_plot on Election Day and Trump Vote Percentage
```{r}

```

## Difference in Means for Rt_plot on Trump Percent and Biden Percent
```{r}
Rt_diff_trump <- lm(cc_nov_4$Rt_plot ~ cc_nov_4$trump.percent, data = cc_nov_4)
summary(Rt_diff_trump)

Rt_diff_biden <-  lm(cc_nov_4$Rt_plot ~ cc_nov_4$biden.percent, data = cc_nov_4)
