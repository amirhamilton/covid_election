...
## Plot of Rt_plot on Election Day
```{r}
...
Here, I merged the data from the various excel spreadsheets from the US (minus Alaska and DC)
```{r, message=FALSE}
alabama_president <- read_excel("data/state_election_results/alabama_president.xlsx") %>%
  mutate(state = "Alabama") %>%
  mutate(dispID = paste(county, state, sep = ", "))
...
relationship between r on eletion day and trump votes- not sure wht this is
```{r, echo = FALSE}
...
##find correlation between county_nov_4$Rt_plot and county_nov_4$trump.percent
```
